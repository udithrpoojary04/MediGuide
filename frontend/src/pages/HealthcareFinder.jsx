                    ...
                    >
                      <div className="flex justify-between items-start">
                        <div className="pr-2">
                          ...
                          <span className="inline-block mt-1 px-2 py-0.5 bg-slate-100 text-slate-600 text-xs font-semibold rounded capitalize">{facility.type}</span>
                        ...
                        <div className="flex flex-col items-end shrink-0">
                          <span className="inline-flex items-center justify-center rounded-full bg-primary-50 px-2.5 py-1 text-xs font-bold text-primary-700 border border-primary-100">
                            {facility.distance} km
                          </span>
                          <ChevronRight className="w-4 h-4 mt-3 text-slate-300 group-hover:text-primary-400 group-hover:translate-x-1 transition-all" />
                        </div>
                      </div>
                    </li>
                  ))}
            ...
            {userLocation && (
              <Marker position={[userLocation.lat, userLocation.lng]}>
                <Popup className="premium-popup">
                  <div className="font-bold text-slate-800 text-center">Your Location</div>
                </Popup>
              </Marker>
            )}
              ...
              >
                <Popup className="premium-popup">
                  <div className="font-sans min-w-[200px]">
                    <strong className="block text-base mb-2 text-slate-800">{facility.name}</strong>
                    <span className="inline-block px-2 py-1 bg-primary-50 text-primary-700 text-xs font-bold rounded capitalize mb-3">{facility.type}</span>
                    ...
                    <div className="text-sm text-slate-600 space-y-2 mb-4">
                      ...
                      {facility.phone !== 'Not available' && (
                        <div className="flex items-center">
                          ...
                          <a href={`tel:${facility.phone}`} className="text-primary-600 hover:text-primary-700 font-medium hover:underline">{facility.phone}</a>
                        </div>
                      )}
                      
                      {facility.website !== 'Not available' && (
                        <div className="flex items-center">
                          <Globe className="h-4 w-4 mr-2 text-slate-400 shrink-0" />
                          <a href={facility.website} target="_blank" rel="noopener noreferrer" className="text-primary-600 hover:text-primary-700 font-medium hover:underline truncate max-w-[150px] inline-block align-bottom">{facility.website}</a>
                        </div>
                      )}
                    </div>
                    ...
                    <a 
                      ...
                      className="mt-2 flex w-full items-center justify-center text-center bg-gradient-to-r from-primary-500 to-primary-600 text-white rounded-xl px-4 py-2 text-sm font-bold shadow-md hover:shadow-lg transition-all"
                    >
